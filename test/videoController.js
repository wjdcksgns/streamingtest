...
document.addEventListener('DOMContentLoaded', () => {
    ...
    player.on('ended', () => {
        ...
        if (popup1Markers.includes(marker)) {
            console.log(`[ended] 현재 marker: ${marker}`);
            console.log(`[ended] popup1Markers에 포함?`, popup1Markers.includes(marker));
            ...
            console.log('[popup1] adding visible class');
            popup1.classList.add('visible');
            console.log('popup1 offsetHeight:', popup1.offsetHeight);
            console.log('popup1 visibility:', getComputedStyle(popup1).visibility);
            console.log('popup1 display:', getComputedStyle(popup1).display);
            popup1.offsetHeight; // ✅ iOS 레이아웃 강제 반영
        } else {
            showUnity(marker);
        }
    });
